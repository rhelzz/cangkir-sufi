            ...
            <form action="{{ route('categories.update', $category->id) }}" method="POST" autocomplete="off">
                ...
                <div class="mb-6">
                    ...
                    <div class="flex space-x-3">
                        <label class="flex items-center cursor-pointer bg-red-50 border border-red-200 rounded-md px-3 py-2 transition hover:bg-red-100 shadow-sm {{ old('type', $category->type) == 'hot' ? 'ring-2 ring-red-400' : '' }}">
                            <input type="radio" name="type" value="hot" {{ old('type', $category->type) == 'hot' ? 'checked' : '' }} 
                        ...
                        <label class="flex items-center cursor-pointer bg-blue-50 border border-blue-200 rounded-md px-3 py-2 transition hover:bg-blue-100 shadow-sm {{ old('type', $category->type) == 'cold' ? 'ring-2 ring-blue-400' : '' }}">
                            <input type="radio" name="type" value="cold" {{ old('type', $category->type) == 'cold' ? 'checked' : '' }} 
