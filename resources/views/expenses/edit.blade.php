...
<div class="max-w-lg mx-auto px-2 py-3">
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <div class="flex justify-between items-center px-4 py-3 border-b">
            <h2 class="text-base font-bold">Edit Expense</h2>
            ...
            <form action="{{ route('expenses.update', $expense->id) }}" method="POST" autocomplete="off">
                ...
                <div class="mb-3">
                    ...
                    <div id="items-container" class="flex flex-col gap-2">
                        @foreach($expense->items as $index => $item)
                            <div class="flex gap-2 items-center item-row" data-index="{{ $index }}">
                                ...
                                <input type="text" name="items[{{ $index }}][item_name]" placeholder="Item name" 
                                    ...
                                    required>
                                <input type="number" name="items[{{ $index }}][price]" placeholder="Price" min="0" step="0.01"
                                    value="{{ $item->price }}"
                ...
                <div class="flex items-center justify-between mt-6 border-t pt-3">
                    <div class="text-sm font-semibold">
                        Total: <span id="total-amount" class="text-blue-600">
                            Rp {{ number_format($expense->items->sum('price'), 0, ',', '.') }}
                        </span>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let itemCount = {{ $expense->items->count() }};
